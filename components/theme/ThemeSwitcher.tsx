...
export function ThemeSwitcher() {
  ...
  useEffect(() => {
    ...
    if (!seen) {
      // set asynchronously to avoid calling setState synchronously inside effect
      const start = setTimeout(() => setShowToast(true), 0);
      localStorage.setItem("portfolio-theme-seen", "1");
      const end = setTimeout(() => setShowToast(false), 4000);
      return () => {
        clearTimeout(start);
        clearTimeout(end);
      };
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        aria-label={
          isOpen ? "Close theme switcher" : "Open theme switcher (press T)"
        }
