...
import {
  ...
  Search, Briefcase, Code2, Database,
  Brain, Cloud, Terminal, GitBranch,
} from 'lucide-react'
...
const INFO_ITEMS: InfoItem[] = [
  ...
  { icon: Mail,           label: 'Email',    value: 'nayandeep1412@gmail.com', href: 'mailto:nayandeep1412@gmail.com' },
  { icon: Phone,          label: 'Phone',    value: '+91-9121684888',           href: 'tel:+919121684888' },
...
function InfoCard({ item, index }: { item: InfoItem; index: number }) {
  ...
  const inner = (
    ...
    >
      ...
      >
        <Icon size={16} style={{ color: 'var(--accent-1)' }} strokeWidth={1.8} />
      </div>
  ...
  return (
    <motion.div key={item.label} {...slideRight(index * 0.07)}>
      {item.href
        ? <a href={item.href} data-cursor="link" className="block">{inner}</a>
        : inner
      }
    </motion.div>
  )
...
const LEARN_ITEMS: LearnItem[] = [
  { icon: Code2,     title: 'Data Structures & Algorithms', desc: 'Building a strong problem-solving foundation'  },
  { icon: Database,  title: 'DBMS',                         desc: 'Learning database design and SQL'              },
  { icon: Brain,     title: 'Artificial Intelligence',      desc: 'Exploring ML concepts and applications'        },
  { icon: Cloud,     title: 'Cloud Computing',              desc: 'AWS fundamentals and cloud architecture'        },
  { icon: Terminal,  title: 'SQL',                          desc: 'Structured query language and databases'       },
  { icon: GitBranch, title: 'Git & GitHub',                 desc: 'Version control and collaboration workflows'   },
]
    ...
    <motion.div {...fadeUp(0.1 + index * 0.06)}>
      ...
      >
        <div className="flex items-start gap-3">
          ...
          >
            <Icon size={15} style={{ color: 'var(--accent-1)' }} strokeWidth={1.8} />
          </div>
          <div>
            <h4
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-[1fr_1.05fr] gap-12 lg:gap-20 items-start">
          ...
          <div className="space-y-8">
            ...
            <motion.div {...fadeUp(0.1)} className="space-y-4">
              <p style={{ color: 'var(--text-secondary)', lineHeight: 1.8 }}>
                Nayan Deep is a Computer Science student currently pursuing{' '}
                <strong style={{ color: 'var(--text-primary)' }}>B.Tech at Desh Bhagat University</strong>,
                with a foundation built through a{' '}
                <strong style={{ color: 'var(--text-primary)' }}>Diploma in CSE from NIMS University, Jaipur</strong>.
              </p>
              <p style={{ color: 'var(--text-secondary)', lineHeight: 1.8 }}>
                He is passionate about{' '}
                <strong style={{ color: 'var(--accent-1)' }}>Software Development</strong>,{' '}
                <strong style={{ color: 'var(--accent-1)' }}>Artificial Intelligence</strong>,{' '}
                <strong style={{ color: 'var(--accent-1)' }}>Cloud Computing</strong>,{' '}
                <strong style={{ color: 'var(--accent-1)' }}>Data Analytics</strong>, and{' '}
                <strong style={{ color: 'var(--accent-1)' }}>Database Systems</strong> — and is
                actively building skills through hands-on projects and certifications.
              </p>
            </motion.div>
          ...
          <div className="space-y-6">
            ...
            >
              <div className="flex items-start gap-3">
                <GraduationCap size={20} style={{ color: 'var(--accent-2)', flexShrink: 0, marginTop: 2 }} strokeWidth={1.8} />
                <div>
                  ...
                  <p className="text-xs mt-1" style={{ color: 'var(--text-secondary)' }}>
                    ...
                    <strong style={{ color: 'var(--text-primary)' }}>NIMS University, Jaipur</strong>
                  </p>
                </div>
              </div>
            </motion.div>
