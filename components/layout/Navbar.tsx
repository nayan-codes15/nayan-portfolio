...
const NAV_ITEMS: NavItem[] = [
  ...
  { id: 'github',         label: 'GitHub', external: 'https://github.com/nayan-codes15' },
...
const SOCIAL_LINKS = [
  { href: 'https://github.com/nayan-codes15',                  Icon: GithubIcon,   label: 'GitHub'   },
  { href: 'https://www.linkedin.com/in/nayan-deep-460119279', Icon: LinkedinIcon, label: 'LinkedIn' },
...
const MOBILE_MENU = {
  ...
  visible: { opacity: 1, clipPath: 'circle(200% at calc(100% - 44px) 36px)',
...
const SOCIAL_ITEM = (i: number) => ({
  ...
  animate:   { opacity: 1, y: 0, transition: { delay: 0.5 + i * 0.07, type: 'spring' as const, stiffness: 280, damping: 24 } },
})

// ── ND Logo ───────────────────────────────────────────────────
function Logo({ onClick, scrolled }: { onClick?: () => void; scrolled?: boolean }) {
  return (
    <Link href="/" aria-label="Nayan Deep — Home" onClick={onClick}>
      ...
      >
        ...
        {/* Name text */}
        <div className={`flex-col leading-none transition-all duration-300 ${scrolled ? 'hidden' : 'hidden sm:flex'}`}>
          <span
...
// ── Desktop nav link ──────────────────────────────────────────
function DeskLink({ item, active, onNavigate }: {
  item: NavItem; active: boolean; onNavigate: (item: NavItem) => void
}) {
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'text-[var(--accent-1)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]',
      )}
    >
...
function MobileMenu({
  isOpen, onClose, active, onNavigate,
}: {
  isOpen: boolean; onClose: () => void; active: string; onNavigate: (item: NavItem) => void
}) {
  // Lock body scroll
  useEffect(() => {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          variants={MOBILE_MENU} initial="hidden" animate="visible" exit="exit"
        ...
        >
          ...
          {/* Nav links */}
          <motion.nav variants={LINK_CONTAINER} initial="hidden" animate="visible" className="flex-1">
              ...
              {NAV_ITEMS.map((item) => (
                <motion.li key={item.id} variants={LINK_ITEM}>
                  <button
                    ...
                    style={{
                      color: active === item.id ? 'var(--accent-1)' : 'var(--text-primary)',
                    }}
                  >
                    <span
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <a
              ...
              style={{ color: 'var(--accent-1)', border: '1px solid var(--accent-1)' }}
            >
              <Download size={13} />
              Resume
            </a>
