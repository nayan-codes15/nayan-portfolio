...
// Deterministic seeded RNG to avoid impure Math.random usage during render
...
function generateNodes(count = 14) {
...
const FloatingShape = memo(function FloatingShape({
  ...
  const geo = useMemo(() => {
    switch (cfg.type) {
      case "icosahedron":
        return <icosahedronGeometry args={[cfg.size, 1]} />;
      case "octahedron":
        return <octahedronGeometry args={[cfg.size]} />;
      case "box":
        return <boxGeometry args={[cfg.size, cfg.size, cfg.size]} />;
    }
  }, [cfg.type, cfg.size]);
...
function ParallaxScene({ color }: { color: string }) {
  ...
  return (
    <group ref={groupRef}>
      {SHAPES.map((cfg, i) => (
        <FloatingShape key={i} cfg={cfg} color={color} />
      ))}
